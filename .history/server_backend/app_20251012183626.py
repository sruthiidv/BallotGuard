...
def create_sample_data():
    ...
    try:
        ...
        print("📈 Initial vote distribution:")
...
def create_app():
    ...
    # NEW: Vote creation endpoint
    @api_bp.route('/votes', methods=['POST'])
    def create_vote():
        """Create new vote - REAL-TIME DEMO FEATURE"""
        try:
            data = request.get_json()
            ...
            vote = Vote(
                ...
                election_id=data['election_id'],
                timestamp=datetime.now(),
                blockchain_hash=f"hash_{data['voter_id']}_{int(time.time())}"
            )
            ...
            updated_candidate = Candidate.query.get(data['candidate_id'])
            
            print(f"🗳️ NEW VOTE RECORDED: {data['voter_id']} → {candidate.name}")
    ...
    @api_bp.route('/elections/<int:election_id>/stats', methods=['GET'])
    def get_election_stats(election_id):
        ...
        try:
            ...
            total_votes = sum(stat.vote_count for stat in vote_stats)
            ...
            return jsonify({
                ...
                'turnout_percentage': round((total_votes / election.eligible_voters * 100), 2),
...
if __name__ == "__main__":
    try:
        ...
        print("📈 Real-time stats: http://localhost:5000/api/elections/1/stats")
        print("=" * 60)
