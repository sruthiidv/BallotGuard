...
class AdminPanel:
    ...
    def create_security_monitor(self, notebook):
        ...
        # Chain restoration
        restore_frame = Labelframe(frame, text="Chain Restoration", padding=10)
        restore_frame.pack(fill="x", pady=20)
        
        Label(restore_frame, text="If chain is broken, it requires system restart:", 
              font=("Arial", 10)).pack()
        
        Button(restore_frame, text="🔄 Check Chain Status", 
               command=self.check_chain_status, bootstyle="info").pack(pady=5)
        
        # Security log
        log_frame = Labelframe(frame, text="Security Audit Log", padding=10)
        log_frame.pack(fill="both", expand=True, pady=20)
        
        self.security_log = scrolledtext.ScrolledText(log_frame, height=8, 
                                                    bg="#1a1a1a", fg="#00ff00",
                                                    font=("Consolas", 10))
...
⚠️ System requires restart to restore chain integrity!""")
    ...
    def test_delete_votes(self):
        ...
        success, message, violation = self.blockchain.delete_votes(10)
...
⚠️ System requires restart to restore chain integrity!""")
    ...
    def test_reset_chain(self):
        ...
        success, message, violation = self.blockchain.reset_chain()
...
⚠️ System requires immediate restart!""")
    ...
    def check_chain_status(self):
        """Check current blockchain chain status"""
        chain_status = self.blockchain.get_chain_status()
        
        if chain_status['broken']:
            messagebox.showerror("Chain Status", 
                               f"""⛓️ BLOCKCHAIN CHAIN STATUS: BROKEN

🔴 Status: {chain_status['status']}
⚠️ Integrity: COMPROMISED
📊 Total Blocks: {chain_status['total_blocks']}
🔒 Values: PRESERVED but UNTRUSTED

The blockchain chain has been compromised due to
administrator modification attempts.

System restart required to restore integrity.""")
        else:
            messagebox.showinfo("Chain Status", 
                              f"""⛓️ BLOCKCHAIN CHAIN STATUS: SECURE

✅ Status: {chain_status['status']}
🔒 Integrity: VERIFIED
📊 Total Blocks: {chain_status['total_blocks']}

All blockchain operations functioning normally.""")
    ...
    def update_chain_status(self):
        ...
        if chain_status['broken']:
            ...
            self.election_status.config(text="🔴 Election Active | Blockchain COMPROMISED", 
                                      bootstyle="danger")
            self.integrity_status.config(text="🚨 Chain Integrity COMPROMISED", 
                                       bootstyle="danger")
        else:
            self.chain_status_label.config(text="🟢 BLOCKCHAIN SECURE", bootstyle="success")
            self.election_status.config(text="🟢 Election Active | Blockchain Secure", 
                                      bootstyle="success")
            self.integrity_status.config(text="✅ Chain Integrity Verified", 
                                       bootstyle="success")
    ...
    def refresh_data(self):
        ...
        try:
            ...
            # Update displays (values remain intact even if chain is broken)
            ...
            self.log_security(f"⚠️ Data refresh error: {e}")
...
💾 File Status: {'✅ Found' if blockchain_info.get('blockchain_file_exists') else '⚠️ Not Found'}
⛓️ Chain Status: {'🔴 BROKEN' if blockchain_info.get('chain_broken') else '✅ SECURE'}
🔒 Integrity: {'⚠️ COMPROMISED' if not blockchain_info.get('integrity_verified') else '✅ VERIFIED'}"""
            ...
            recent_blocks = self.blockchain.get_recent_blocks(15)
            ...
            self.log_security(f"⚠️ Blockchain refresh error: {e}")
        ...
        try:
            ...
            if filename:
                ...
                export_data = {
                    ...
                    "data_integrity_warning": "Chain broken - data preserved but untrusted" if chain_status['broken'] else "Data integrity verified"
                }
                ...
                if chain_status['broken']:
                    messagebox.showwarning("Export Complete with Warning", 
                                         f"""⚠️ Results exported with integrity warning!

✅ File saved: {filename}
⚠️ Chain Status: BROKEN
🔒 Data: Preserved but untrusted

Note: Blockchain chain has been compromised.
Exported data should be verified independently.""")
                else:
    ...
    def run(self):
        ...
        print("⚠️ Chain Protection: Any modification will break chain")
        self.app.mainloop()
