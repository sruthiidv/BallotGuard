...
# Use centralized database folder
...
# --- Ledger Receipts ---
def store_receipt(vote_id: str, election_id: str, idx: int, bhash: str, sig_b64: str | None, db_path=None):
    if db_path is None:
        db_path = DEFAULT_DB_PATH
    con = connect(db_path)
...
def fetch_last_receipt(election_id: str, db_path=None):
...
# --- OVTs ---
def store_ovt(token: str, election_id: str, db_path=None):
    if db_path is None:
        db_path = DEFAULT_DB_PATH
    con = connect(db_path)
    con.execute(
...
def mark_ovt_used(token: str, db_path=None):
    if db_path is None:
        db_path = DEFAULT_DB_PATH
    con = connect(db_path)
...
def is_ovt_used(token: str, db_path=None) -> bool:
    if db_path is None:
        db_path = DEFAULT_DB_PATH
    con = connect(db_path)
...
# --- Encrypted Votes ---
def store_vote(vote_id: str, election_id: str, encrypted_vote: str, ovt_token: str, db_path=None):
    if db_path is None:
        db_path = DEFAULT_DB_PATH
    con = connect(db_path)
    con.execute(
...
def fetch_encrypted_votes(election_id: str, db_path=None):
    if db_path is None:
        db_path = DEFAULT_DB_PATH
    con = connect(db_path)
