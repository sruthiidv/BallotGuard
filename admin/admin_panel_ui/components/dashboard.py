...
from tkinter.ttk import Frame, Label, Button, LabelFrame as Labelframe
...
class Dashboard(Frame):
    ...
    def setup_ui(self):
        ...
        left_panel = Labelframe(main_frame, text="Vote Statistics")
        ...
        # Candidate breakdown
        candidates_frame = Labelframe(left_panel, text="Candidate Breakdown")
        ...
        right_panel = Labelframe(main_frame, text="Admin Controls")
        right_panel.pack(side="right", fill="y", padx=(10, 0))
        
        # Control buttons
        Button(right_panel, text="🔄 Refresh Data", 
               command=self.update_all_data).pack(pady=5, padx=10, fill="x")
        
        Button(right_panel, text="📊 Export Results", 
               command=self.export_results).pack(pady=5, padx=10, fill="x")
        
        Button(right_panel, text="🔍 Test Connection", 
               command=self.test_connection).pack(pady=5, padx=10, fill="x")
        
        Button(right_panel, text="⚙️ System Health", 
               command=self.show_system_health).pack(pady=5, padx=10, fill="x")
        
        Button(right_panel, text="📋 View Logs", 
               command=self.show_logs).pack(pady=5, padx=10, fill="x")
        
        # Bottom status bar
        self.status_bar = Label(self, text="Ready")
        self.status_bar.pack(side="bottom", fill="x", pady=(10, 0), padx=10)
