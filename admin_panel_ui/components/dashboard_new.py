...
class Dashboard(ttk.Frame):
    ...
    def setup_ui(self):
        # Title
        title = ttk.Label(self, text="BallotGuard Admin Dashboard", style="title.TLabel")
        title.pack(pady=20)
        ...
        left_panel = ttk.LabelFrame(main_frame, text="Vote Statistics", padding=20)
        left_panel.pack(side="left", fill="both", expand=True, padx=(0, 10))
        
        self.total_votes_label = ttk.Label(left_panel, text="Total Votes: Loading...", 
                                      style="total.TLabel")
        self.total_votes_label.pack(pady=10)
        
        self.status_label = ttk.Label(left_panel, text="Status: Checking...", 
                                 style="info.TLabel")
        self.status_label.pack(pady=5)
        
        self.last_update_label = ttk.Label(left_panel, text="Last Updated: --", 
                                      style="small.TLabel")
        self.last_update_label.pack(pady=5)
        
        # Candidate breakdown
        candidates_frame = ttk.LabelFrame(left_panel, text="Candidate Breakdown", padding=10)
        ...
        # Control buttons
        ttk.Button(right_panel, text="🔄 Refresh Data", 
               command=self.update_all_data).pack(pady=5, fill="x")
        
        ttk.Button(right_panel, text="📊 Export Results", 
               command=self.export_results, style="success.TButton").pack(pady=5, fill="x")
        
        ttk.Button(right_panel, text="🔍 Test Connection", 
               command=self.test_connection, style="info.TButton").pack(pady=5, fill="x")
        
        ttk.Button(right_panel, text="⚙️ System Health", 
               command=self.show_system_health, style="warning.TButton").pack(pady=5, fill="x")
        
        ttk.Button(right_panel, text="📋 View Logs", 
