...
# Use database storage for elections
from server_backend.db.elections import (
    get_all_elections, get_election, create_election, 
    update_election_status, add_candidate
)
...
def election_endpoints():
    ...
    if request.method == 'GET':
        ...
        return jsonify(get_all_elections())
    else:
        """Create new election"""
        try:
            data = request.json
            ...
            block_obj = blockchain_mod.Block(
                index=0,
                timestamp=time.time(),
                vote_hash=sha_utils.compute_sha256_hex(f'GENESIS||{new_election["election_id"]}'),
            ...
            c.execute("INSERT INTO ledger_blocks (election_id, ledger_index, vote_hash, prev_hash, hash, ts) VALUES (?, ?, ?, ?, ?, ?)",
                     (new_election["election_id"], 0, block_obj.vote_hash, block_obj.previous_hash, block_obj.hash, block_obj.timestamp))
            conn.commit()
            conn.close()
...
def get_election_endpoint(election_id):
...
def verify_blockchain(election_id):
    """Verify blockchain for specific election"""
    try:
        conn = get_db()
        c = conn.cursor()
        ...
        for block in blocks:
            ...
            block_obj = blockchain_mod.Block(
                index=block_dict["ledger_index"],
                timestamp=block_dict["ts"],
                vote_hash=block_dict["vote_hash"],
            ...
            # Verify block
            if block_dict["prev_hash"] != prev_hash:
                response["status"] = "tampered"
                response["message"] = f"Break in hash chain at block {block_dict['ledger_index']}"
                block_status["is_valid"] = False
            
            if block_dict["hash"] != block_obj.hash:
                response["status"] = "tampered"
                response["message"] = f"Invalid block hash at block {block_dict['ledger_index']}"
                block_status["is_valid"] = False
            
            response["blocks"].append(block_status)
...
def simulate_tampering(election_id):
    """Simulate tampering with the blockchain by modifying a random block's hash"""
    try:
        conn = get_db()
        c = conn.cursor()
        
        # Get all blocks for election
        c.execute("SELECT ledger_index FROM ledger_blocks WHERE election_id=? AND ledger_index > 0", (election_id,))
        blocks = c.fetchall()
        
        if not blocks:
            return jsonify({"error": {"code": "NOT_FOUND", "message": "No blocks found in election"}}), 404

        # Select a random non-genesis block
        import random
        target_block = random.choice(blocks)
        target_index = target_block["ledger_index"]
        
        # Tamper with the block by modifying its hash
        tampered_hash = "TAMPERED_" + sha_utils.compute_sha256_hex(str(time.time()))
        c.execute("""
            UPDATE ledger_blocks 
            SET hash = ? 
            WHERE election_id = ? AND ledger_index = ?""", 
            (tampered_hash, election_id, target_index))
        
        conn.commit()
        conn.close()
        
        return jsonify({
            "message": f"Successfully simulated tampering at block {target_index}",
            "tampered_block": target_index
        })

    except Exception as e:
        return jsonify({"error": {"code": "TAMPER_FAILED", "message": str(e)}}), 500
